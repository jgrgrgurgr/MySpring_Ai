...
# class StrokePredictor:  # 이름은 유지했으나 기능은 포즈 예측으로 변경
...
#             image = ImageOps.fit(image, (256, 256), Image.LANCZOS)  # MoveNet 입력 크기
#             image_array = np.asarray(image, dtype=np.uint8)  # 0~255 범위
#             image_array = tf.cast(image_array, tf.int32)  # MoveNet 기대 타입
#             input_data = tf.expand_dims(image_array, axis=0)  # (1, 256, 256, 3)
...
#             # TFLite 모델 입력 준비
#             input_data = np.expand_dims(flattened.numpy(), axis=0).astype(np.float32)  # (1, 34)
...
#             output_data_float = (current_features.astype(np.float32) - output_zero_point) * output_scale  # (1, 2)
            
#             # 두 클래스 확률 계산
#             probabilities = output_data_float[0]  # [prob_class0, prob_class1]
#             predicted_class_idx = np.argmax(probabilities)
#             class_name = self.class_names[predicted_class_idx]
            
#             # stroke_probability와 유사한 개념으로 최대 확률 사용
#             pose_probability = float(probabilities[predicted_class_idx])
#             non_pose_probability = 1.0 - pose_probability
            
#             return {
#                 "filename": "unknown",
#                 "class": class_name,
#                 "stroke_probability": pose_probability,  # 첫 번째 클래스 확률 (이름 유지)
#                 "non_stroke_probability": non_pose_probability,  # 두 번째 클래스 확률 (이름 유지)
#                 "severity_score": int(round(pose_probability * 100))  # 기존 정의 유지
#             }
#         except Exception as e:
#             return {
#                 "filename": "unknown",
...
#     # 기존 extract_features와 predict_image는 사용하지 않으므로 생략 가능
#     # 필요하면 파일 경로 기반 예측을 위해 유지할 수 있음
...
# def ai_send():
#     try:
#         print("Request received:", request.method, request.headers)  # 요청 정보 출력
#         print("Files in request:", request.files)  # 파일 정보 출력
#         print("JSON in request:", request.is_json, request.get_json(silent=True))  # JSON 정보 출력

#         if 'image' in request.files:
#             file = request.files['image']
#             print("File received:", file, "Filename:", file.filename)  # 파일 정보 출력
#             if file.filename == '' or not file:
#                 return jsonify({"status": "error", "message": "Invalid image file"}), 400
#             image_data = file.read()
#         elif request.is_json:
#             data = request.get_json()
#             print("JSON data:", data)  # JSON 데이터 출력
#             if 'image' in data:
#                 try:
